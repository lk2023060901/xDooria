...
// DefaultReporterConfig 默认配置（保障最小可用）
...
func (r *Reporter) report() {
	...
	metadata := map[string]string{
		...
		"connections":  fmt.Sprintf("%d", stats.ActiveConnections),
		// 系统资源
		...
		// 带宽
		"bytes_in_per_sec":  fmt.Sprintf("%.2f", stats.BytesInPerSec),
		"bytes_out_per_sec": fmt.Sprintf("%.2f", stats.BytesOutPerSec),
		// 时间戳
