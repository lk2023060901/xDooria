...
type SessionManager interface {
	...
	// Broadcast 向所有会话广播数据，返回失败数量。
	Broadcast(ctx context.Context, env *common.Envelope) (failed int)
	// Close 关闭并清空所有会话。
	Close() error
}
...
// Broadcast 向所有会话广播数据，返回失败数量。
func (m *BaseSessionManager) Broadcast(ctx context.Context, env *common.Envelope) (failed int) {
	m.mu.RLock()
...
func (m *BaseSessionManager) Close() error {
	...
	if failed > 0 {
		return fmt.Errorf("%w: %d sessions", ErrCloseFailed, failed)
	}
	return nil
}